...
app.get('/api/me', requireAuth, async (req, res) => {
  res.json({ id: req.user.id, email: req.user.email, pro: await isPro(req.user.id, req.user.email) });
});
...
// -------------------- AI risk narration --------------------
app.post('/api/explain', optionalAuth, async (req, res) => {
...
app.post('/api/briefing', optionalAuth, async (req, res) => {
  if (!process.env.ANTHROPIC_API_KEY) return res.status(501).json({ error: 'ai_not_configured' });
