  ...
  $('#statsRow').innerHTML = `
    <div class="stat-card score-card">
      <svg width="64" height="64" viewBox="0 0 64 64">
        ...
        <circle cx="32" cy="32" r="26" fill="none" stroke="${scoreColor}" stroke-width="4" stroke-linecap="round"
          stroke-dasharray="${circumference}" stroke-dashoffset="${offset}" transform="rotate(-90 32 32)"
          style="transition: stroke-dashoffset 1s cubic-bezier(0.16,1,0.3,1); filter:drop-shadow(0 0 6px ${scoreColor})"/>
        <text x="32" y="38" text-anchor="middle" font-family="Orbitron" font-weight="700" font-size="18" fill="${scoreColor}">${score === null ? '--' : score}</text>
      </svg>
      <div class="stat-label">SECURITY SCORE</div>
    </div>
    <div class="stat-card"><div class="stat-num">${total}</div><div class="stat-label">HOSTS FOUND</div></div>
    <div class="stat-card"><div class="stat-num" style="color:var(--high)">${counts.high}</div><div class="stat-label">HIGH RISK</div></div>
    <div class="stat-card"><div class="stat-num" style="color:var(--medium)">${counts.medium}</div><div class="stat-label">MEDIUM RISK</div></div>
    <div class="stat-card"><div class="stat-num" style="color:var(--low)">${counts.low}</div><div class="stat-label">LOW RISK</div></div>
  `;
}
...
function renderFindingsTable(nodes) {
  $('#findingsTable').innerHTML = nodes.map(n => `
    <div class="finding-row" data-host="${n.id}">
      <span class="finding-risk-dot" style="background:${n.type === 'root' ? '#39d6ff' : (RISK_COLOR[n.risk] || '#7f96b8')}"></span>
