...
function bindAuthEvents() {
  ...
  // Sidebar icon rail — every icon now does something real, not decorative.
  $('#railSurface').addEventListener('click', () => {
    document.querySelectorAll('.rail-icon').forEach(i => i.classList.remove('active'));
    $('#railSurface').classList.add('active');
  });
  $('#railDomains').addEventListener('click', () => {
    document.querySelectorAll('.rail-icon').forEach(i => i.classList.remove('active'));
    $('#railDomains').classList.add('active');
    openDomainsModal();
  });
  $('#railReports').addEventListener('click', () => {
    if (!lastScanData) { toast('Run a scan first — nothing to export yet.'); return; }
    exportReport();
  });
  $('#railSettings').addEventListener('click', () => $('#settingsModal').classList.add('show'));
  $('#closeSettingsModal').addEventListener('click', () => $('#settingsModal').classList.remove('show'));

  $('#themeDark').addEventListener('click', () => setTheme('dark'));
  $('#themeLight').addEventListener('click', () => setTheme('light'));

  $('#searchBtn').addEventListener('click', () => { $('#domainSelect').focus(); toast('Pick a domain to scan.'); });
  $('#bellBtn').addEventListener('click', () => {
    toast(lastScanData ? `Last scan: ${lastScanData.domain} — ${lastScanData.nodes.length} hosts` : 'No scans yet today.');
  });
}

function setTheme(mode) {
  document.documentElement.setAttribute('data-theme', mode);
  localStorage.setItem('sentramap_theme', mode);
...
function renderGraph(data) {
  ...
  const others = data.nodes.filter(n => n.type !== 'root');
  const byRing = { high: [], medium: [], low: [] };
  others.forEach(n => (byRing[n.risk] || byRing.low).push(n));

  const maxDim = Math.min(width, height);
  const RINGS = [
    { key: 'high', r: maxDim * 0.22 },
    { key: 'medium', r: maxDim * 0.34 },
    { key: 'low', r: maxDim * 0.44 }
  ];

  const nodes = [{ ...rootNode, x: cx, y: cy }];
  RINGS.forEach(ring => {
    const group = byRing[ring.key];
    group.forEach((n, i) => {
      const angle = (i / Math.max(group.length, 1)) * 2 * Math.PI - Math.PI / 2;
      nodes.push({ ...n, x: cx + Math.cos(angle) * ring.r, y: cy + Math.sin(angle) * ring.r });
    });
  });

  const links = others.map((n, i) => ({ source: rootNode.id, target: n.id, phase: (i * 0.29) % 1 }));
  const nodeById = new Map(nodes.map(n => [n.id, n]));

  // Background concentric ring guides, matching the reference proximity-map aesthetic.
  const ringGuides = svg.append('g');
  RINGS.forEach(ring => {
    ringGuides.append('circle').attr('cx', cx).attr('cy', cy).attr('r', ring.r)
      .attr('fill', 'none').attr('stroke', 'rgba(57,214,255,0.1)').attr('stroke-width', 1);
  });

  // Large center readout — the signature element from the reference proximity map.
  const centerLabel = svg.append('g').style('opacity', 0);
  centerLabel.append('text').attr('x', cx).attr('y', cy + 38).attr('text-anchor', 'middle')
    .attr('font-family', 'Orbitron').attr('font-weight', '900').attr('font-size', '22')
    .attr('fill', 'rgba(228,238,252,0.9)').text(others.length + 1);
  centerLabel.append('text').attr('x', cx).attr('y', cy + 52).attr('text-anchor', 'middle')
    .attr('font-family', 'JetBrains Mono').attr('font-size', '8').attr('letter-spacing', '1.5')
    .attr('fill', 'rgba(127,150,184,0.8)').text('HOSTS MAPPED');
  centerLabel.transition().delay(600).duration(500).style('opacity', 1);

  const link = svg.append('g').selectAll('line').data(links).enter().append('line')
    .attr('class', 'link')
    .attr('x1', d => nodeById.get(d.source).x).attr('y1', d => nodeById.get(d.source).y)
    .attr('x2', d => nodeById.get(d.target).x).attr('y2', d => nodeById.get(d.target).y);
  ...
  node.filter(d => d.type !== 'root').append('text').attr('class', 'node-label').attr('dy', 15 + 16).attr('text-anchor', 'middle')
    .text(d => d.label.length > 22 ? d.label.slice(0, 20) + '…' : d.label);

  node.filter(d => d.type === 'root').each(function () {
    const g = d3.select(this);
    [0, 1, 2].forEach(i => {
      g.insert('circle', ':first-child').attr('class', 'ping-ring').attr('r', 20)
        .attr('fill', 'none').attr('stroke', '#39d6ff').attr('stroke-width', 1.5)
        .html(`<animate attributeName="r" values="20;70" dur="3.6s" begin="${i * 1.2}s" repeatCount="indefinite"/>
                <animate attributeName="opacity" values="0.55;0" dur="3.6s" begin="${i * 1.2}s" repeatCount="indefinite"/>`);
    });
  });

  // Continuous data-flow pulse animation from center outward — reinforces "live system."
  function animatePulses() {
    const t = (Date.now() / 1300) % 1;
    pulses.attr('cx', d => { const s = nodeById.get(d.source), e = nodeById.get(d.target); return s.x + (e.x - s.x) * ((t + d.phase) % 1); })
          .attr('cy', d => { const s = nodeById.get(d.source), e = nodeById.get(d.target); return s.y + (e.y - s.y) * ((t + d.phase) % 1); })
          .attr('fill', d => RISK_COLOR[nodeById.get(d.target).risk] || '#39d6ff')
...
// ============== Init ==============
async function init() {
  setTheme(localStorage.getItem('sentramap_theme') || 'dark');
  bindAuthEvents();
