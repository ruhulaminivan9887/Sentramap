...
function drawGaugeTicks() {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    const x1 = 75 + Math.cos(angle) * 68, y1 = 75 + Math.sin(angle) * 68;
    const x2 = 75 + Math.cos(angle) * 72, y2 = 75 + Math.sin(angle) * 72;
    ticks.push(`<line class="gauge-tick" x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}"/>`);
  }
  g.innerHTML = ticks.join('');
}

function setGaugeScanning(isScanning) {
  const sweep = $('#gaugeSweep');
  $('#gaugeWrap').style.display = 'block';
  if (isScanning) {
    sweep.classList.remove('settled');
    $('#gaugeScore').textContent = '--';
    $('#gaugeFill').setAttribute('stroke-dashoffset', GAUGE_CIRCUMFERENCE);
  } else {
    sweep.classList.add('settled');
  }
}

function updateGauge(score) {
  const color = score >= 80 ? '#39ff9d' : score >= 50 ? '#ffb84d' : '#ff3b5c';
...
async function runScan() {
  ...
  try {
    ...
    const data = await res.json();
    if (res.status === 402) { $('#paywallModal').classList.add('show'); status.classList.remove('show'); setGaugeScanning(false); clearInterval(termInterval); return; }
    if (data.error) { status.textContent = `Error: ${data.message}`; setGaugeScanning(false); clearInterval(termInterval); termLog(`ERROR: ${data.message}`); return; }
...
async function fetchBriefing(data) {
  ...
  try {
    ...
    if (res.status === 402) { setGaugeScanning(false); return; }
    const briefing = await res.json();
    setGaugeScanning(false);
    if (briefing.score !== undefined) updateGauge(briefing.score);
...
function renderGraph(data) {
  ...
  // Reticle / target-lock look instead of a plain filled dot.
  node.append('circle').attr('class', 'node-ring').attr('r', d => d.type === 'root' ? 26 : 16).attr('stroke', colorFor);
  node.each(function (d) {
    const r = d.type === 'root' ? 26 : 16;
  ...
  node.append('text').attr('class', 'node-label').attr('dy', d => (d.type === 'root' ? 26 : 16) + 16).attr('text-anchor', 'middle')
