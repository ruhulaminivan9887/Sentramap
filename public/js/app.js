...
async function runScan() {
  ...
  try {
    ...
    const data = await res.json();
    if (res.status === 402) { $('#paywallModal').classList.add('show'); status.classList.remove('show'); setGaugeScanning(false); return; }
    if (data.error) { status.textContent = `Error: ${data.message}`; setGaugeScanning(false); return; }
